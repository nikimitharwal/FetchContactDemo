package com.nikita.contactfetch.ContentViewModel
...
class ContactsViewModel(val mapp: Application) : AndroidViewModel(mapp) {
    ...
    fun fetchContacts() {
        viewModelScope.launch {
            val contactAllList = async { getAllContacts() }
            val contactnumList = async { getAllNumbers() }
            val contactmailList = async { getAllEmail() }

            val contacts = contactAllList.await()
            val contactNumbers = contactnumList.await()
            val contactEmails = contactmailList.await()
    ...
    private suspend fun getAllContacts(): ArrayList<Contact> {
        val contactsList = ArrayList<Contact>()
        val contactsCursor = mapp.contentResolver?.query(
    ...
    private suspend fun getAllNumbers(): HashMap<String, ArrayList<String>> {
        val contactsNumberMap = HashMap<String, ArrayList<String>>()
        val phoneCursor: Cursor? = mapp.contentResolver.query(
        ...
        if (phoneCursor != null && phoneCursor.count > 0) {
            ...
            while (phoneCursor.moveToNext()) {
                ...
                //if map contain key or not then create a new array list with number
    ...
    private suspend fun getAllEmail(): HashMap<String, ArrayList<String>> {
        val contactsEmailMap = HashMap<String, ArrayList<String>>()
        val emailCursor = mapp.contentResolver.query(ContactsContract.CommonDataKinds.Email.CONTENT_URI,
                null,
                null,
                null,
                null)
        if (emailCursor != null && emailCursor.count > 0) {
            ...
            while (emailCursor.moveToNext()) {
                ...
                //if map contain key or  not then create a new array list with email
